#ifndef BITCOIN_CSCRIPT_H
#define BITCOIN_CSCRIPT_H
...
inline string ValueString(const vector<unsigned char>& vch) {
    if (vch.size() <= 4) {
        return strsprintf("%d", CBigNum(vch).getint());
    } else {
...
inline string StackString(const vector<vector<unsigned char>>& vStack) {
    ...
    BOOST_FOREACH(const vector<unsigned char>& vch, vStack) {
        if (!str.empty()) {
            str += " ";
        }
        str += ValueString(vch);
